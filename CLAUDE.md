...
npm run build
npm run test          # Vitest
npm run review:svg    # SVG visual review HTML生成
...
`npm run review:svg` は、APIリクエスト形式のサンプル入力からSVGを生成し、入力JSONと出力SVGを一覧できる静的HTMLを作成する。長期保守するvisual regression testではなく、描画バグを目視で探すための一時レビュー用ツール。

出力先: `test_output_dir/svg-visual-review/index.html`
